...
function ListPage() {
  ...
  return (
    <div className="space-y-8">
      <PageHeader eyebrow="Billing" title="Bookings" description="Confirmed plot bookings and agreements."
        actions={<Button size="lg"><Plus className="size-4 mr-1.5" />New</Button>} />
      <div className="rounded-2xl border bg-card overflow-hidden shadow-sm">
        {rows.length === 0 ? (
          <EmptyState icon={<BookCheck className="size-5" />} title="No bookings yet" />
        ) : (
