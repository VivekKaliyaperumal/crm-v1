...
const opts = queryOptions({ queryKey: ["campaigns"], queryFn: () => listCampaigns() });
...
function ListPage() {
  ...
  return (
    <div className="space-y-8">
      <PageHeader eyebrow="Growth" title="Campaigns" description="Marketing campaigns driving leads."
        actions={<Button size="lg"><Plus className="size-4 mr-1.5" />New</Button>} />
      <div className="rounded-2xl border bg-card overflow-hidden shadow-sm">
        {rows.length === 0 ? (
          <EmptyState icon={<Megaphone className="size-5" />} title="No campaigns yet" />
        ) : (
