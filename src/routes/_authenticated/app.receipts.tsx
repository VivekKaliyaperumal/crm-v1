...
const opts = queryOptions({ queryKey: ["receipts"], queryFn: () => listReceipts() });
...
function ListPage() {
  ...
  return (
    <div className="space-y-8">
      <PageHeader eyebrow="Billing" title="Receipts" description="Issued receipts for payments."
        actions={<Button size="lg"><Plus className="size-4 mr-1.5" />New</Button>} />
      <div className="rounded-2xl border bg-card overflow-hidden shadow-sm">
        {rows.length === 0 ? (
          <EmptyState icon={<Receipt className="size-5" />} title="No receipts yet" />
        ) : (
